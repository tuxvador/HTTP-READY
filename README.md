...
Progress messages and results are printed as plain lines — the script emits no terminal escape sequences at all, so scrollback, mouse wheel, `shift+PageUp`, terminal search and piping to a file all behave exactly as they do for any other command.

An earlier version pinned the progress messages to a fixed on-screen region using the VT100 scroll-margin escape (DECSTBM). It kept status and results visually apart, but how a terminal treats lines scrolled out of a *partial* scroll region varies between emulators: in several of them those lines never reach the scrollback, so results that scrolled off the top could not be recovered. Since this tool's whole output is a list you need to read back through, plain output is the right trade — losing results off-screen costs more than interleaved status lines do.
...
- **Plain output**: removed the pinned status panel and its scroll-region escapes. Lines scrolled out of a partial DECSTBM region do not reach the scrollback in every terminal, so results could scroll off-screen unrecoverably; output is now ordinary lines and scrollback works everywhere
